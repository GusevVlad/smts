...
# This script can be executed from the root directory and prints ready-to-use curl commands
...
# Function to send message to EXT SMTS with curl command
send_to_ext_smts() {
...
EOF
    ...
    echo "=== Ready-to-use curl command for EXT SMTS ==="
    ...
    if [ "$HTTP_CODE" -eq 200 ] || [ "$HTTP_CODE" -eq 201 ]; then
        echo "✅ Message sent successfully to EXT SMTS!"
        return 0
    else
...
# Function to send message to INT SMTS with curl command
send_to_int_smts() {
    local message_id=$(generate_message_id)
    local timestamp=$(date -u +%Y-%m-%dT%H:%M:%SZ)
...
EOF
    ...
    echo "=== Ready-to-use curl command for INT SMTS ==="
    ...
    if [ "$HTTP_CODE" -eq 200 ] || [ "$HTTP_CODE" -eq 201 ]; then
        echo "✅ Message sent successfully to INT SMTS!"
        return 0
    else
...
check_received_messages() {
    ...
    echo "=== Ready-to-use curl command for checking $description ==="
    ...
    if [ "$RECEIVED_COUNT" -gt 0 ]; then
        echo "✅ Message successfully transported!"
        return 0
    else
...
echo
echo "1. Send message to EXT SMTS:"
echo "   curl -X POST \"$API_BASE_URL_EXT/send/$TOPIC_MONTERRA\" \\"
echo "     -H \"Content-Type: application/json\" \\"
echo "     -H \"X-API-Key: $API_KEY\" \\"
...
echo
echo "2. Send message to INT SMTS:"
echo "   curl -X POST \"$API_BASE_URL_INT/send/$TOPIC_MONTERRA\" \\"
echo "     -H \"Content-Type: application/json\" \\"
echo "     -H \"X-API-Key: $API_KEY\" \\"
echo "     -H \"X-SMTS-Message-ID: \$(date +%Y%m%d%H%M%S)-\$(openssl rand -hex 4)\" \\"
